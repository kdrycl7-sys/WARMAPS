(() => {
  ...
  let selected = conflicts[0], map, layer, busy = false;
  ...
  function renderTabs() { $('tabs').innerHTML = conflicts.map(c => `<button class="tab" type="button" aria-selected="${c.id === selected.id}" data-id="${c.id}">${escape(c.label)}</button>`).join(''); $('tabs').querySelectorAll('button').forEach(button => button.addEventListener('click', () => { selected = conflicts.find(c => c.id === button.dataset.id); render(); loadFeed(); })); }
  function renderMap() {
  ...
  async function loadFeed() {
    ...
    const items = results.flatMap(result => result.status === 'fulfilled' ? result.value : []).filter(item => item.title && item.link).sort((a,b) => new Date(b.published || 0) - new Date(a.published || 0)).slice(0,20);
    $('feed').innerHTML = items.length ? items.map(item => `<article class="feed-item"><h3><a href="${escape(item.link)}" target="_blank" rel="noopener noreferrer">${escape(item.title)}</a><span class="badge">kaynak bildirimi</span></h3><div class="meta">${escape(item.source)} · ${escape(formatDate(item.published))}</div></article>`).join('') : '<div class="state error">Canlı akış alınamadı veya boş döndü. Proxy/CORS ayarlarını ve kaynakları kontrol edin; demo haber üretilmedi.</div>';
    $('feed-status').textContent = items.length ? `${items.length} başlık` : 'Akış yok'; busy = false; $('updated').textContent = `Son güncelleme: ${formatDate(new Date())}`;
  }
  function render() { renderTabs(); renderMap(); renderVideos(); }
